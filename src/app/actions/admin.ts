...
export async function enterResult(formData: FormData) {
  ...
  // Knockouts: push the winner into the next round automatically.
...
/**
 * After a knockout result is entered, place the winner into the correct
 * next-round fixture. Match IDs are deterministic: `ko-<ROUND>-<index>`.
 * Match i feeds next-round match ceil(i/2), home slot if i is odd else away.
 * Semi-final losers drop into the third-place match. Draws are skipped
 * (the admin resolves a penalty shootout by assigning teams manually).
 */
async function advanceKnockoutWinner(matchId: string) {
